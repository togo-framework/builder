...
`))),t.appendChild(m)}else r++}return t}var jn=/(`+)([\s\S]*?)\1|\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)|(\*\*|__)([\s\S]+?)\5|(~~)([\s\S]+?)\7|(\*|_)([^\s*_][\s\S]*?)\9|(https?:\/\/[^\s<>()]+)/;function ge(e){let t=document.createDocumentFragment(),n=e;for(;;){let r=jn.exec(n);if(!r||r.index===void 0)break;if(r.index>0&&Pt(t,n.slice(0,r.index)),r[1]){let o=document.createElement("code");o.className="md-code",o.textContent=r[2].trim(),t.appendChild(o)}else r[3]!==void 0?t.appendChild(At(r[4],r[3]||r[4])):r[5]?t.appendChild(Fe("strong","md-strong",r[6])):r[7]?t.appendChild(Fe("del","md-del",r[8])):r[9]?t.appendChild(Fe("em","md-em",r[10])):r[11]&&t.appendChild(At(r[11],r[11]));n=n.slice(r.index+r[0].length)}return n&&Pt(t,n),t}function Fe(e,t,n){let r=document.createElement(e);return r.className=t,r.appendChild(ge(n)),r}function At(e,t){if(!(/^(https?:|mailto:)/i.test(e)||/^[/#]/.test(e)))return document.createTextNode(t);let r=document.createElement("a");return r.className="md-a",r.href=e,r.target="_blank",r.rel="noopener noreferrer ugc",r.textContent=t,r}function Pt(e,t){t.split(`
